...
# Phase 20.22: Uiverse-inspired Country/City combobox
from uiverse_combobox import CountryCitySelector
...
def send_appcommand(hwnd: int, command: int) -> bool:
    ...
    # Phase 20.12: use correct WM_APPCOMMAND lParam with device/key flags.
    # 0xC000 indicates FAPPCOMMAND_OEM and ensures the command is recognized
    # by players like VLC that require the full lParam encoding.
    lparam = (command << 16) | 0xC000
    ok = bool(SendMessageTimeoutW(hwnd, WM_APPCOMMAND, hwnd, lparam, SMTO_ABORTIFHUNG, 200, ctypes.byref(result)))
...
def win7_browser_fallback(player_name: str) -> bool:
    ...
    Firefox (its in-window APPCOMMAND handler works)."""
    return windows_is_7() and Path(player_name).stem.casefold() in WIN7_BROWSER_STEMS


def needs_media_key_fallback(player_name: str) -> bool:
    stem = Path(player_name).stem.casefold()
    if stem in WIN7_BROWSER_STEMS and windows_is_7():
        return True
    return False
...
def load_settings() -> dict:
    default = {
        ...
        "autostart": False,
        # Phase 47-B: location settings (automatic preferred, no silent fallback).
        "location_mode": "auto", "manual_city": "", "manual_country": "",
    ...
    try:
        ...
        # Phase 20.28: one-time migration of the stale manual location state.
        # Earlier builds persisted a manual location (e.g. Jeddah/Saudi Arabia,
        # sometimes with coordinates) that the location UI then re-displayed on
        # every startup and reverted to after each fetch. Clear it once and let
        # the mode fall back to the default (auto); the marker prevents a later
        # re-migration from wiping a location the user deliberately chose.
        if not loaded.get("location_migrated_20_28"):
            loaded["location_mode"] = "auto"
            loaded["manual_city"] = ""
            loaded["manual_country"] = ""
            loaded["manual_latitude"] = ""
            loaded["manual_longitude"] = ""
            loaded["manual_timezone"] = ""
            loaded["location_migrated_20_28"] = True
            migrated = True
...
# Phase 75 REDESIGN: Windows 11 / Fluent Design palette.
# Light = bright neutral surfaces with a single blue accent (Windows 11 light).
# Dark = near-black Mica-like surfaces with a refined blue accent.
# Every key used anywhere in the app is preserved; only values are upgraded.
# Semantic colors (success/warning/error) are tuned for the surface they sit on.
THEMES = {
    "light": {
        "bg": "#F5F5F7", "surface": "#FFFFFF", "surface_alt": "#F2F2F7",
        "border": "#D1D1D6", "border_soft": "#E5E5EA",
        "text": "#000000", "muted": "#6D6D70", "accent": "#0078D4",
        "accent_hover": "#106EBE", "accent_text": "#FFFFFF",
        "success": "#10B981", "warning": "#FFB800", "error": "#C42B1C",
        "disabled_fg": "#A0A0A0", "disabled_field": "#E0E0E0",
        "focus": "#0078D4", "hover_field": "#F0F0F0",
        "chip_bg": "#E0E0E0", "hero_bg": "#FFFFFF",
        "sidebar": "#F0F0F0", "sidebar_active": "#FFFFFF",
        "sidebar_text": "#000000", "ring_track": "#D1D1D6",
        "countdown_text": "#000000",
        "card_shadow": "#0000001A",
    },
    "dark": {
        "bg": "#202024", "surface": "#29292D", "surface_alt": "#2D2D30",
        "border": "#3D3D42", "border_soft": "#323236",
        "text": "#FFFFFF", "muted": "#A0A0A0", "accent": "#4A90E2",
        "accent_hover": "#6BB0FF", "accent_text": "#000000",
        "success": "#3FDB9B", "warning": "#FFC000", "error": "#E7453F",
        "disabled_fg": "#6D6D70", "disabled_field": "#202024",
        "focus": "#4A90E2", "hover_field": "#3D3D42",
        "chip_bg": "#3D3D42", "hero_bg": "#29292D",
        "sidebar": "#1E1E22", "sidebar_active": "#29292D",
        "sidebar_text": "#FFFFFF", "ring_track": "#3D3D42",
...
FONTS = {
    # Segoe UI is the native Windows 11 face and is available on Win7+.
    # Sizes follow the Fluent type ramp (12/14/18/22).
    "display": ("Segoe UI", 22, "bold"),
    "title": ("Segoe UI", 14, "bold"),
    "body": ("Segoe UI", 12),
    "caption": ("Segoe UI", 11),
    "countdown": ("Consolas", 30, "bold"),
    "countdown_big": ("Consolas", 48, "bold"),
    "mono": ("Consolas", 11),
}

# Fluent corner radius hierarchy: 8px controls, 12px cards, 16px hero.
CARD_RADIUS = 12
CARD_RADIUS_HERO = 16

# Segoe UI Symbol glyphs are available on Windows 7+ and need no new assets.
ICONS = {
...
class ThemeToggle(tk.Frame):
    """Custom sun/moon toggle for light/dark mode.
    
    Native Tkinter Canvas implementation adapted from Uiverse.io by mobinkakei.
    Recreates the sun/moon toggle appearance natively without HTML/CSS.
    """
    
    def __init__(self, parent, command=None, dark=False):
        super().__init__(parent)
        self.command = command
        self.dark = dark
        self._animating = False
        
        # AM/PM labels like Uiverse design
        self.am_label = tk.Label(self, text="AM", font=("Segoe UI", 10, "bold"), bg=self.cget("bg"))
        self.pm_label = tk.Label(self, text="PM", font=("Segoe UI", 10, "bold"), bg=self.cget("bg"))
        
        # Toggle canvas 90x50
        self.canvas = tk.Canvas(self, width=90, height=50, highlightthickness=0, bd=0, bg=self.cget("bg"))
        
        # Layout: AM | canvas | PM
        self.am_label.pack(side="left", padx=(0, 6))
        self.canvas.pack(side="left")
        self.pm_label.pack(side="left", padx=(6, 0))
        ...
        # Initial colors
        if dark:
            self._track_color = "#749dd6"
            self._handler_color = "#ffe5b5"
            self._label_color = "#749ed7"
            self.am_label.config(fg="#749ed7")
            self.pm_label.config(fg="#ffffff")
        else:
            self._track_color = "#83d8ff"
            self._handler_color = "#ffcf96"
            self._label_color = "#749ed7"
            self.am_label.config(fg="#ffffff")
            self.pm_label.config(fg="#749ed7")
        
        self._handler_x = 67.0 if dark else 23.0
        self._draw()
    ...
    def toggle(self):
        # Compatibility method - just trigger command
        self._on_click()
    
    def set_dark(self, dark: bool, animate: bool = True):
        """Set dark mode state. Animate transition if animate=True."""
        if self.dark == dark and not animate:
            return
        # Skip if already animating to target
        if self._animating and self.dark == dark:
            return
        ...
        target_x = 67.0 if dark else 23.0
        target_track = "#749dd6" if dark else "#83d8ff"
        target_handler = "#ffe5b5" if dark else "#ffcf96"
        
        if not animate:
            self._handler_x = target_x
            self._track_color = target_track
            self._handler_color = target_handler
            self._draw()
            return
        
        # Animate from current state to target
        self._anim_from_x = self._handler_x
        self._anim_to_x = target_x
        self._anim_from_track = self._track_color
        self._anim_to_track = target_track
        self._anim_from_handler = self._handler_color
        self._anim_to_handler = target_handler
    ...
    def _anim_frame(self):
        ...
        self._handler_x = self._anim_from_x + (self._anim_to_x - self._anim_from_x) * eased
        self._track_color = self._lerp_color(self._anim_from_track, self._anim_to_track, eased)
        self._handler_color = self._lerp_color(self._anim_from_handler, self._anim_to_handler, eased)
        
        self._draw()
        
        if progress < 1.0:
            self.after(16, self._anim_frame)
        else:
            self._animating = False
            # Ensure final state
            self._handler_x = self._anim_to_x
            self._track_color = self._anim_to_track
            self._handler_color = self._anim_to_handler
            self._draw()
    
    def _draw_rounded_rect(self, x1, y1, x2, y2, r, fill):
        """Draw rounded rectangle approximation."""
        # Simple approach: draw rect with corner ovals
        self.canvas.create_rectangle(x1+r, y1, x2-r, y1, x2, y2, fill=fill, outline="", width=0)
        self.canvas.create_rectangle(x1, y1+r, x1, y2-r, x2, y2, fill=fill, outline="", width=0)
        # Corners
        self.canvas.create_arc(x1, y1, x1+2*r, y1+2*r, start=90, extent=90, fill=fill, outline="", style=tk.PIESLICE)
        self.canvas.create_arc(x2-2*r, y1, x2, y1+2*r, start=0, extent=90, fill=fill, outline="", style=tk.PIESLICE)
        self.canvas.create_arc(x1, y2-2*r, x1+2*r, y2, start=180, extent=90, fill=fill, outline="", style=tk.PIESLICE)
        self.canvas.create_arc(x2-2*r, y2-2*r, x2, y2, start=270, extent=90, fill=fill, outline="", style=tk.PIESLICE)
    ...
    def _draw(self):
        ...
        # Using simple rectangle approximation for performance on Win7
        self.canvas.create_rectangle(0, 0, w, h, fill=self._track_color, outline="", width=0)
        # Add rounded mask by drawing smaller rect and corners
        # Simplified: just rectangle for now, performance over perfect corners
        
        # Draw stars (dark mode only)
        if self.dark:
            # Small stars that appear when dark
            star_positions = [(35, 10), (28, 18), (40, 27), (11, 16), (17, 32), (28, 36)]
            for x, y in star_positions:
                size = 2 if x not in [35, 28, 40] else 3
                self.canvas.create_oval(x, y, x+size, y+size, fill="#ffffff", outline="")
        
        # Handler position
        cx = int(self._handler_x)
        cy = h // 2
        r = 20
        
        # Handler shadow
        self.canvas.create_oval(cx-r+2, cy-r+2, cx+r+2, cy+r+2, fill="#000000", outline="", stipple="gray25")
        
        # Handler circle
        self.canvas.create_oval(cx-r, cy-r, cx+r, cy+r, fill=self._handler_color, outline="", width=0)
        
        # Sun rays or moon craters
        if not self.dark:
            # Sun rays - draw 8 lines radiating out
            import math
            for angle in range(0, 360, 45):
                ...
                self.canvas.create_line(x1, y1, x2, y2, fill="#ffcf96", width=2, capstyle="round")
        else:
            # Moon craters
            craters = [(10, -7, 4), (22, 8, 6), (-25, -10, 8)]
            for dx, dy, size in craters:
                cxr = cx + dx
                cyr = cy + dy
                self.canvas.create_oval(cxr-size, cyr-size, cxr+size, cyr+size, fill="#e8cda5", outline="")
...
def apply_theme(style: ttk.Style, theme: str) -> None:
    """Phase 75 REDESIGN: Windows 11 / Fluent Design styling.

    Applies the complete visual system to every Tk widget class. The theme is
    deliberately Windows-11-inspired: neutral surfaces, a single blue accent,
    subtle borders, and clear semantic states. All legacy style names are kept
    so existing widgets continue to work.
    """
    dark = theme != "vista"
    ...
    # Card-surface variants of Title/Hint — a plain "TLabel" always paints the
    # PAGE background (bg) behind its text, which shows as a mismatched patch
    # once labels sit inside a rounded surface/surface_alt card. Same size/
    # weight/color as Title.TLabel and Hint.TLabel, just matched to the card
    # they're actually placed on.
    ...
    style.configure("SoftHint.TLabel", foreground=muted, font=FONTS["caption"],
                    background=surface_alt)
    style.configure("Status.TLabel", foreground=accent, font=("Segoe UI", 11, "bold"))
    ...
    # Semantic button variants (danger/warning) — new, additive.
    style.configure("Danger.TButton", font=("Segoe UI", 11, "bold"),
                    background=t["error"], foreground=t["accent_text"],
                    bordercolor=t["error"], padding=(12, 6))
    ...
    style.configure("Warning.TButton", font=("Segoe UI", 11, "bold"),
                    background=t["warning"], foreground=t["accent_text"],
                    bordercolor=t["warning"], padding=(12, 6))
    ...
    style.configure("Nav.TButton", background=t["sidebar"],
                    ...
                    focuscolor=t["sidebar"], font=("Segoe UI", 11),
    ...
    style.configure("NavActive.TButton", background=t["sidebar_active"],
                    ...
                    font=("Segoe UI", 11, "bold"), padding=(14, 14),
...
class App:
    ...
    def show_splash(self) -> None:
        ...
        try:
            ...
            splash.update()
            self._splash = splash
            # Phase 20.2 FIX: delay close timer to ensure splash is visible
            # Schedule close after splash is actually visible
    ...
    def build_ui(self) -> None:
        ...
        # ============ ROOT SHELL: top navigation + main area ============
        ...
        # ---------------- TOP NAVIGATION BAR ----------------
        top_bar = ttk.Frame(shell, style="TFrame", padding=(12, 8, 12, 8))
        ...
        # brand left
        brand = ttk.Frame(top_bar, style="TFrame")
        brand.grid(row=0, column=0, sticky="w")
        ttk.Label(brand, text="◈ Prayer Music Guard", style="SidebarLogo.TLabel", anchor="w").pack(side="left")
        ttk.Label(brand, text="صلاة وسكون", style="SidebarFoot.TLabel", anchor="w").pack(side="left", padx=(8, 0))

        # navigation buttons - grid-based, responsive
        nav_frame = ttk.Frame(top_bar, style="TFrame")
        nav_frame.grid(row=0, column=1, sticky="ew")
        self._nav_frame = nav_frame
        nav_frame.columnconfigure(tuple(range(5)), weight=1)
        self._nav_buttons = {}
        self._nav_bars = {}
        ...
        for idx, (key, label) in enumerate(nav_items):
            container = ttk.Frame(nav_frame, style="TFrame")
            container.grid(row=0, column=idx, sticky="nsew", padx=3)
            btn = ttk.Button(container, text=label, style="Nav.TButton",
                             command=lambda k=key: self.show_page(k))
            btn.pack(fill="x")
            bar = tk.Frame(container, height=3, background=self.tokens["sidebar"])
        ...
        # right side controls
        right = ttk.Frame(top_bar, style="TFrame")
        right.grid(row=0, column=2, sticky="e")
        self.monitor_chip = ttk.Label(right, text="● المراقبة متوقفة", style="ChipAlt.TLabel", anchor="w")
        self.monitor_chip.pack(side="left", padx=(0, 8))
        self.theme_toggle = ThemeToggle(right, command=self.toggle_theme, dark=self.theme_choice == "dark")
        self.theme_toggle.pack(side="left", padx=(0, 8))
        self.header_version_chip = ttk.Label(right, text=f"v{APP_VERSION}", style="ChipAlt.TLabel", anchor="w")
        self.header_version_chip.pack(side="left")

        # ---------------- MAIN AREA ----------------
        main = ttk.Frame(shell, style="TFrame", padding=(16, 12, 16, 12))
        main.grid(row=1, column=0, sticky="nsew")
        main.columnconfigure(0, weight=1)
        main.rowconfigure(1, weight=1)

        # ===== PAGE HEADER (title + subtitle + date) =====
        header = ttk.Frame(main, style="Header.TFrame")
        header.grid(row=0, column=0, sticky="ew")
        header.columnconfigure(0, weight=1)
        hsub = ttk.Frame(header, style="Header.TFrame")
        hsub.pack(fill="x", pady=(0, 8))
        hsub.columnconfigure(0, weight=1)
        self.header_title = ttk.Label(hsub, text="Prayer Music Guard", style="HeaderTitle.TLabel", anchor="w")
        ...
        self.header_sub = ttk.Label(header, text="صلاة وسكون — حماية الموسيقى وقت الصلاة", style="HeaderSub.TLabel", anchor="w")
        self.header_sub.pack(fill="x", pady=(0, 8))
        # divider
        ...
        # ---- HERO BAND: full-width stacked hero card ----
        # Countdown text stacked ON TOP of ring canvas — never clipped,
        # true responsive layout where content flows vertically.
        ...
        ring_info.grid(row=0, column=0, sticky="nsew", padx=(20, 20), pady=(16, 8))
        ring_info.columnconfigure(0, weight=1)
        self.next_prayer_label = ttk.Label(ring_info, text="—", style="HeroTitle.TLabel", anchor="center")
        self.next_prayer_label.pack(fill="x", pady=(0, 4))
        self.next_prayer_time = ttk.Label(ring_info, text="--:--", style="CountdownTime.TLabel", anchor="center")
        self.next_prayer_time.pack(fill="x", pady=(0, 4))
        self.next_prayer_in = ttk.Label(ring_info, text="00:00:00", style="CountdownBig.TLabel", anchor="center")
        self.next_prayer_in.pack(fill="x", pady=(4, 8))
        ttk.Label(ring_info, text="الوقت المتبقي — Time Remaining", style="CardHint.TLabel", anchor="center").pack(fill="x")
        ttk.Label(ring_info, text="يقترب موعد الصلاة", style="CardHint.TLabel", anchor="center", foreground=self.tokens["muted"]).pack(fill="x", pady=(2, 0))
        self.ring_canvas = tk.Canvas(ring_card, width=240, height=240,
                                       highlightthickness=0, background=self.tokens["surface"])
        self.ring_canvas.grid(row=1, column=0, sticky="nsew", padx=(24, 24), pady=(0, 16))
        ring_card.rowconfigure(1, weight=1)

        # ---- TODAY'S TIMES: responsive prayer cards grid ----
        # Truly responsive: column count adapts to available width.
        # Narrow: 2 cols, Medium: 3 cols, Wide: 5 cols.
        ...
        times_grid.pack(fill="both", expand=True, padx=14, pady=14)
        self.dash_time_labels = {}
        self._times_grid = times_grid
        self._times_prayer_widgets = {}
        for prayer in PRAYERS:
            card = ttk.Frame(times_grid, style="Card.TFrame")
            card.columnconfigure(0, weight=1)
            name_lbl = ttk.Label(card, text=AR[prayer], style="Card.TLabel", anchor="center", font=("Segoe UI", 12, "bold"))
            name_lbl.pack(fill="x", pady=(0, 4))
            time_lbl = ttk.Label(card, text="--:--", style="PrayerTime.TLabel", anchor="center", font=("Segoe UI", 14, "bold"))
            time_lbl.pack(fill="x", pady=(0, 4))
            self.dash_time_labels[prayer] = (name_lbl, time_lbl)
            self._times_prayer_widgets[prayer] = card
        self._update_prayer_grid(1)

        # ---- LOWER CARDS: truly responsive grid ----
        # Columns change based on available width: narrow=1, medium=2, wide=3
        ...
        status_outer.grid(row=0, column=0, sticky="nsew", padx=(0, 6))
        ...
        music_outer.grid(row=0, column=1, sticky="nsew", padx=6)
        ...
        quick_outer.grid(row=0, column=2, sticky="nsew", padx=(6, 0))
        ...
        # Phase 47-B: location section — Uiverse-inspired Country/City selector
        ...
        # Mode radio buttons
        mode_frame = ttk.Frame(loc, style="Card.TFrame")
        mode_frame.pack(fill="x", pady=(0, 8))
        ttk.Radiobutton(mode_frame, text="تحديد الموقع تلقائيًا (تقريبي عبر IP)", value="auto", variable=self.location_mode).pack(anchor="e")
        ttk.Radiobutton(mode_frame, text="استخدام الموقع اليدوي", value="manual", variable=self.location_mode).pack(anchor="e")
        
        # Country/City selector (Uiverse-style)
        self.country_city_selector = CountryCitySelector(
            loc,
            country_var=self.manual_country,
            city_var=self.manual_city,
            on_country_change=self._on_manual_country_change,
            on_city_change=self._on_manual_city_change,
            theme=self.theme_choice
        )
        self.country_city_selector.pack(fill="x", pady=(0, 8))
        
        # Lat/Long/Timezone (optional manual coordinates)
        coords_frame = ttk.Frame(loc, style="Card.TFrame")
        coords_frame.pack(fill="x", pady=(0, 8))
        coords_frame.columnconfigure((1, 3), weight=1)
        ttk.Label(coords_frame, text="خط العرض:", style="Card.TLabel").grid(row=0, column=0, sticky="e")
        self.manual_latitude_entry = ttk.Entry(coords_frame, textvariable=self.manual_latitude, width=10, justify="center")
        self.manual_latitude_entry.grid(row=0, column=1, sticky="ew", padx=(0, 6))
        ttk.Label(coords_frame, text="خط الطول:", style="Card.TLabel").grid(row=0, column=2, sticky="e")
        self.manual_longitude_entry = ttk.Entry(coords_frame, textvariable=self.manual_longitude, width=10, justify="center")
        self.manual_longitude_entry.grid(row=0, column=3, sticky="ew")
        ttk.Label(coords_frame, text="المنطقة الزمنية (اختياري):", style="Card.TLabel").grid(row=1, column=0, columnspan=2, sticky="e", pady=(4, 0))
        self.manual_timezone_entry = ttk.Entry(coords_frame, textvariable=self.manual_timezone, width=24, justify="center")
        self.manual_timezone_entry.grid(row=1, column=2, columnspan=2, sticky="ew", pady=(4, 0))
        
        lbtns = ttk.Frame(times_card, style="Card.TFrame")
        lbtns.grid(row=len(PRAYERS) + 2, column=0, sticky="ew", pady=(6, 0))
        ...
        ttk.Button(times_card, text=f"{ICONS['refresh']}  جلب المواقيت", command=self.fetch_times).grid(row=len(PRAYERS) + 3, column=0, sticky="e", pady=(6, 0))
        ...
        self.root.after(100, lambda: (
            self._update_prayer_grid(self.root.winfo_width()),
            self._update_nav_layout(self.root.winfo_width())
        ))
    ...
    def apply_theme_selection(self) -> None:
        ...
        # Phase 20.22: update Country/City selector theme
        if getattr(self, "country_city_selector", None) is not None:
            self.country_city_selector.set_theme(self.theme_choice)
        # Update theme toggle visual state
        if getattr(self, "theme_toggle", None) is not None:
            self.theme_toggle.set_dark(self.theme_choice == "dark")
    ...
    def _on_root_configure(self, event: tk.Event) -> None:
        ...
        try:
            ...
            pass
        # Reflow page cards using page canvas width
        page_canvas = getattr(self, "_cards_canvas", None)
        page_w = page_canvas.winfo_width() if page_canvas else event.width
        self._reflow_cards(page_w)
        # Reflow top navigation using root width
        self._update_nav_layout(event.width)

    def _update_prayer_grid(self, width: int) -> None:
        """Set prayer times grid column count based on available width."""
        times_grid = getattr(self, "_times_grid", None)
        widgets = getattr(self, "_times_prayer_widgets", {})
        if times_grid is None or not widgets:
            return
        try:
            # Determine column count: narrow=2, medium=3, wide=5
            if width < 700:
                ncol = 2
            elif width < 1000:
                ncol = 3
                ...
                ncol = 5
            times_grid.columnconfigure(tuple(range(ncol)), weight=1)
            # Clear any old column configs beyond ncol
            for c in range(ncol, 6):
                try:
                    times_grid.columnconfigure(c, weight=0)
                except tk.TclError:
                    pass
            # Place each prayer card in the correct column
            idx = 0
            for prayer, w in widgets.items():
                col = idx % ncol
                row = idx // ncol
                w.grid_configure(row=row, column=col, sticky="nsew", padx=(0 if col==0 else 8, 0), pady=(0, 8))
                idx += 1
        except Exception:
            pass

    def _update_nav_layout(self, width: int) -> None:
        """Reflow top navigation: 5 cols wide, 2 cols medium, 1 col narrow."""
        nav_frame = getattr(self, "_nav_frame", None)
        ...
        try:
            if width >= 900:
                ...
                ncol = 2
            nav_frame.columnconfigure(tuple(range(ncol)), weight=1)
            for c in range(ncol, 5):
                try:
                    nav_frame.columnconfigure(c, weight=0)
                except tk.TclError:
                    pass
            # Re-grid nav items into rows
            nav_items = [("dashboard", "لوحة الصلاة"), ("times", "المواقيت"),
                         ("music", "المشغل"), ("settings", "الإعدادات"),
                         ("about", "حول البرنامج")]
            containers = getattr(self, "_nav_containers", {})
            for idx, (key, label) in enumerate(nav_items):
                container = containers.get(key)
                if container:
                    container.grid_configure(row=idx // ncol, column=idx % ncol, sticky="nsew", padx=3)
        except Exception:
            pass

    def _reflow_cards(self, width: int) -> None:
        """Phase 65: responsive dashboard. Reflow hero ring size, prayer grid, and lower cards."""
        wide = width >= 1000
        if getattr(self, "_wide_cards", None) == wide:
            pass
        self._wide_cards = wide
        # Ring size adapts to available width
        self._size_ring(190 if wide else 132)
        # Responsive countdown font sizing
        try:
            if hasattr(self, "next_prayer_in"):
                fs_big = 84 if width >= 1200 else 72 if width >= 1000 else 60 if width >= 800 else 48
                fs_time = 34 if width >= 1200 else 30 if width >= 1000 else 26 if width >= 800 else 22
                self.next_prayer_in.configure(font=("Consolas", fs_big, "bold"))
                if hasattr(self, "next_prayer_time"):
                    self.next_prayer_time.configure(font=("Consolas", fs_time, "bold"))
        except Exception:
            pass
        # Prayer times grid reflow
        self._update_prayer_grid(width)
        # Top nav reflow
        self._update_nav_layout(width)
        # Lower cards: 3 columns when wide, stacked when narrow
        row3 = getattr(self, "_dash_row3", None)
        cards = getattr(self, "_dash_row3_cards", {})
        if row3 is not None:
            try:
                if wide:
                    for col_idx, key in enumerate(["status", "music", "quick"]):
                        w = cards.get(key)
                        if w:
                            w.grid_configure(row=0, column=col_idx, sticky="nsew", padx=6, pady=0)
                    for c in range(3):
                        row3.columnconfigure(c, weight=1)
                else:
                    for idx, key in enumerate(["status", "music", "quick"]):
                        w = cards.get(key)
                        if w:
                            w.grid_configure(row=idx, column=0, sticky="nsew", padx=12, pady=(0, 12 if idx<2 else 0))
                    row3.columnconfigure(0, weight=1)
                    for c in [1, 2]:
                        row3.columnconfigure(c, weight=0)
            except Exception:
                pass
    ...
    def _on_manual_country_change(self, country_code: str) -> None:
        """Callback when country is selected in manual mode."""
        # City is automatically cleared by CountryCitySelector
        # Update status
        if country_code:
            country_name = ""
            for c in self.country_city_selector.countries:
                if c["code"] == country_code:
                    country_name = c.get("name_ar", c.get("name_en", country_code))
                    break
            self.status.set(f"تم اختيار الدولة: {country_name}. اختر المدينة الآن.")
        else:
            self.status.set("تم مسح اختيار الدولة.")

    def _on_manual_city_change(self, city_name: str) -> None:
        """Callback when city is selected in manual mode."""
        if city_name:
            country_code = self.manual_country.get()
            country_name = ""
            for c in self.country_city_selector.countries:
                if c["code"] == country_code:
                    country_name = c.get("name_ar", c.get("name_en", country_code))
                    break
            self.status.set(f"تم اختيار الموقع: {city_name}، {country_name}.")
        else:
            self.status.set("تم مسح اختيار المدينة.")
    ...
    def _location_snapshot(self) -> dict:
        ...
        try:
            ...
            longitude = None
        city = self.manual_city.get().strip()
        country = self.manual_country.get().strip()
        return {
    ...
    def _resolve_location(snapshot: dict) -> tuple:
        ...
        Priority: manual mode (with valid city/country or coords) -> automatic IP -> error.
        ...
        if snapshot["mode"] == "manual":
            # Manual mode: require city+country OR lat+lon
            has_names = bool(snapshot.get("city") and snapshot.get("country"))
            has_coords = snapshot.get("latitude") is not None and snapshot.get("longitude") is not None
            if has_names or has_coords:
                label = f"{snapshot['city_label']}، {snapshot['country_label']}" if has_names else f"Lat: {snapshot['latitude']}, Lon: {snapshot['longitude']}"
                return (snapshot["city"], snapshot["country"], snapshot["latitude"], snapshot["longitude"], snapshot["timezone"], label, "manual")
            else:
                # Manual mode but no valid location - return error indicator
                return (None, None, None, None, None, "موقع يدوي غير مكتمل", "manual-invalid")
        ...
        # No automatic location available and manual not ready
        return (None, None, None, None, None, "تعذر تحديد الموقع", "none")
    ...
    def fetch_times(self) -> None:
        ...
        Location priority: manual mode (with valid city/country or coords) -> automatic IP.
        The request runs on a daemon worker thread; every failure path keeps the app responsive and
        ...
        def worker(snapshot: dict):
            ...
            # Handle error cases
            if source == "manual-invalid":
                self.root.after(0, lambda: messagebox.showerror("موقع غير صالح", "يرجى اختيار الدولة والمدينة أولاً."))
                self.root.after(0, lambda: self.status.set("موقع يدوي غير مكتمل. اختر الدولة والمدينة."))
                return
            if source == "none":
                self.root.after(0, lambda: messagebox.showerror("موقع غير صالح", "تعذر تحديد الموقع. يرجى اختيار الدولة والمدينة أو تفعيل التحديد التلقائي."))
                self.root.after(0, lambda: self.status.set("تعذر تحديد الموقع."))
                return
    ...
    def apply_times(self, values: dict, location_label: str = "") -> None:
