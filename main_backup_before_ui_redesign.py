...
THEMES = {
    ...
    # separate design.
    "light": {
        "bg": "#EFF2F1", "surface": "#FFFFFF", "surface_alt": "#F5F8F7",
        "border": "#DDE6E3", "border_soft": "#E8EFEC",
        "text": "#12201C", "muted": "#5B6E68", "accent": "#0C9C7C",
        "accent_hover": "#098267", "accent_text": "#FFFFFF",
        "success": "#12A26B", "warning": "#B9821F", "error": "#C74332",
        "disabled_fg": "#9AA8A3", "disabled_field": "#E6EDEA",
        "focus": "#0C9C7C", "hover_field": "#EBF5F1",
        "chip_bg": "#E1F1EA", "hero_bg": "#FFFFFF",
        "sidebar": "#0F1B17", "sidebar_active": "#1B2E27",
        "sidebar_text": "#B7C7C1", "ring_track": "#DEE7E3",
        "countdown_text": "#12201C",
    },
    "dark": {
        "bg": "#0A0D0C", "surface": "#121613", "surface_alt": "#0E1210",
        "border": "#22322B", "border_soft": "#1A2621",
        "text": "#EEF4F1", "muted": "#8CA098", "accent": "#22E6A7",
        "accent_hover": "#5CF3C4", "accent_text": "#04241E",
        "success": "#3FDB9B", "warning": "#EFB65B", "error": "#F1786C",
        "disabled_fg": "#546258", "disabled_field": "#171E1A",
        "focus": "#22E6A7", "hover_field": "#182420",
        "chip_bg": "#152420", "hero_bg": "#121613",
        "sidebar": "#070907", "sidebar_active": "#141C18",
        "sidebar_text": "#94A69D", "ring_track": "#22322B",
        "countdown_text": "#F4FAF7",
    },
}
...
FONTS = {
    ...
    "display": ("Segoe UI", 21, "bold"),
    "title": ("Segoe UI", 14, "bold"),
    "body": ("Segoe UI", 10),
    "caption": ("Segoe UI", 9),
    "countdown": ("Consolas", 26, "bold"),
    "countdown_big": ("Consolas", 42, "bold"),
    "mono": ("Consolas", 10),
}
...
def apply_theme(style: ttk.Style, theme: str) -> None:
    ...
    style.configure("Nav.TButton", background=t["sidebar"],
                    ...
                    padding=(10, 8), anchor="e", relief="flat")
...
class App:
    ...
    def build_ui(self) -> None:
        ...
        sidebar = ttk.Frame(shell, style="Sidebar.TFrame", padding=(18, 22, 18, 12))
        ...
        for key, label in (
            ...
            btn = ttk.Button(sidebar, text=label, style="Nav.TButton", width=20,
        ...
        # ---------------- MAIN AREA ----------------
        main = ttk.Frame(shell, style="TFrame", padding=(22, 18, 22, 14))
        ...
        hrow = ttk.Frame(header, style="Header.TFrame"); hrow.pack(fill="x", pady=(0, 6))
        ...
        hsub = ttk.Frame(header, style="Header.TFrame"); hsub.pack(fill="x")
        ...
        header_rule = tk.Frame(header, background=self.tokens["accent"], height=2)
        header_rule.pack(fill="x", pady=(10, 0))
        ...
        hero_row.grid(row=0, column=0, sticky="nsew", pady=(0, 12))
        ...
        self.next_prayer_in.grid(row=2, column=0, sticky="ew", pady=(6, 4))
        ttk.Label(ring_info, text="الوقت المتبقي — Time Remaining", style="CardHint.TLabel", anchor="e").grid(row=3, column=0, sticky="ew")
        self.ring_canvas = tk.Canvas(ring_card, width=190, height=190,
                                     highlightthickness=0, background=self.tokens["surface"])
        self.ring_canvas.grid(row=0, column=1, sticky="ns", padx=(28, 6))

        # ---- TODAY'S TIMES: a horizontal strip of compact pills (Phase 71 —
        # was a vertical list sharing a row with the music card; now a
        # full-width strip, one column per prayer, echoing the reference
        # apps' horizontal prayer-table pattern). ----
        times_dash_outer, times_dash = self._make_card(dash, f"{ICONS['times']}  مواقيت اليوم", variant="card")
        times_dash_outer.grid(row=1, column=0, sticky="nsew", pady=(0, 12))
        self.dash_time_labels = {}
        for c, prayer in enumerate(PRAYERS):
            times_dash.columnconfigure(c, weight=1)
            pill = ttk.Frame(times_dash, style="Card.TFrame")
            pill.grid(row=0, column=c, sticky="nsew", padx=(0 if c == 0 else 4, 0))
            name_lbl = ttk.Label(pill, text=AR[prayer], style="Card.TLabel", anchor="center")
            name_lbl.pack(fill="x")
            time_lbl = ttk.Label(pill, text="--:--", style="PrayerTime.TLabel", anchor="center", padding=(0, 4))
        ...
        music_outer.grid(row=0, column=1, sticky="nsew", padx=8)
        ...
        quick_outer.grid(row=0, column=2, sticky="nsew", padx=(8, 0))
        ...
        for r, prayer in enumerate(PRAYERS):
            prow = ttk.Frame(times_card, style="Card.TFrame"); prow.grid(row=r, column=0, sticky="ew", pady=1)
        ...
        self._dash_widgets = (ring_card, times_dash, status_card, music_card)
    ...
    def _reflow_cards(self, width: int) -> None:
        """Phase 65: responsive dashboard. Layout is grid-based; reflow only
        adapts the countdown ring size for narrow widths."""
        wide = width >= int(760 * ui_scale(self.root))
        if self._wide_cards == wide:
            return
        self._wide_cards = wide
        # Ring size adapts to available width; layout itself uses grid weights.
    ...
    def _make_card(self, parent: tk.Widget, title: str, variant: str = "card") -> tuple[tk.Canvas, tk.Frame]:
        ...
        win = outer.create_window(14, 12, window=inner, anchor="nw")
        title_lbl = None
        if title:
        ...
        def redraw(_evt=None):
            try:
                ...
                need_h = inner.winfo_reqheight() + 22
                ...
                outer.itemconfigure(win, width=max(10, w - 28))
            except tk.TclError:
